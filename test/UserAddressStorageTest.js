...
describe('UserAddressStorage contract', () => {
	...
	it('can only be called from OpenQ contract', async () => {
		...
		await expect(contractWithNonOwnerAccount.registerUserAddress(gitHubUsername, address)).to.be.revertedWith('OpenQStorage: Only the current OpenQ version can use this function.');
	});

	// LOGIC
	it('registerUserAddress associate GitHub username to Ethereum address', async () => {
		const UserAddressStorage = await ethers.getContractFactory('UserAddressStorage');
		const userAddressStorage = await UserAddressStorage.deploy();
		...
		// insert new user
		await userAddressStorage.registerUserAddress(gitHubUsername, address);
		...
		await userAddressStorage.registerUserAddress(gitHubUsername, newAddress);
