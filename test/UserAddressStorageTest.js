...
describe("UserAddressStorage contract", () => {
    ...
    it("addUserAddress associate GitHub username to Ethereum address", async () => {
        const UserAddressStorage = await ethers.getContractFactory("UserAddressStorage");
        const userAddressStorage = await UserAddressStorage.deploy();
        ...
        await userAddressStorage.addUserAddress(gitHubUsername, address);
    ...
    it("can only be called from OpenQ contract", async () => {
        ...
        await expect(contractWithNonOwnerAccount.addUserAddress(gitHubUsername, address)).to.be.revertedWith('OpenQStorage: Only the current OpenQ version can use this function.');
    });
});