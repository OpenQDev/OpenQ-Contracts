    ...
    <div class="overlay" v-if="showOracleList" @click="$store.commit('setShowOracleList', false);">
      <div class="card shadow-sm oracle-list" @click.stop>
        <div class="card-body p-2">
          ...
          <div :class="'mt-2 text-left d-flex align-items-center btn btn-' + (activeOracle == null ? 'primary' : 'outline-light text-dark')" @click="$store.commit('setActiveOracle', null); $store.commit('setShowOracleList', false)">
          ...
          <div v-for="oracle in oracles" :class="'mt-2 text-left d-flex align-items-center btn btn-' + (activeOracle == oracle ? 'primary' : 'outline-light text-dark')" @click="$store.commit('setActiveOracle', oracle); $store.commit('setShowOracleList', false)">
